...
import com.timss.pf.service.AccidentEnvisionService;
...
@Service
public class AccidentEnvisionServiceImpl extends WfAbstractServiceImpl<ModuleData> implements AccidentEnvisionService {
    public void fixWFBean(ModuleData bean, SecureUser operator) throws Exception {
        if (StringUtils.isBlank(bean.getBusinessUrl())) {
            bean.setBusinessUrl("pf/acciEnvision/moduleDataDetailPage.do?mode=view&moduleCode=" + bean.getBusinessType() + "&id=" + bean.getBusinessId());
        }
//        String desc = DataFormatUtil.substr(this.serviceUtil.fixExpressionFromBean(bean.getModule().getDescExpression(), bean, operator), Integer.valueOf(200));
        String desc = DataFormatUtil.substr("事故预想记录", 200);
    ...
    public AccidentEnvisionServiceImpl() {
    }
