...
import com.timss.pf.service.WorkPlanService;
import com.timss.pf.service.WorkRecordService;
...
@Service
public class WorkRecordServiceImpl extends WfAbstractServiceImpl<ModuleData> implements WorkRecordService {
    public void fixWFBean(ModuleData bean, SecureUser operator) throws Exception {
        if (StringUtils.isBlank(bean.getBusinessUrl())) {
            bean.setBusinessUrl("pf/workRecord/moduleDataDetailPage.do?mode=view&moduleCode=" + bean.getBusinessType() + "&id=" + bean.getBusinessId());
        }
//        String desc = DataFormatUtil.substr(this.serviceUtil.fixExpressionFromBean(bean.getModule().getDescExpression(), bean, operator), Integer.valueOf(200));
        String desc = DataFormatUtil.substr("工作日志", 200);
