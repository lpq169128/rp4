...
import com.timss.pf.service.WorkArrangementService;
...
@Service
public class WorkArrangementServiceImpl extends WfAbstractServiceImpl<ModuleData> implements WorkArrangementService {
    public void fixWFBean(ModuleData bean, SecureUser operator) throws Exception {
        if (StringUtils.isBlank(bean.getBusinessUrl())) {
            bean.setBusinessUrl("pf/workArrangement/moduleDataDetailPage.do?mode=view&moduleCode=" + bean.getBusinessType() + "&id=" + bean.getBusinessId());
        }
    ...
    public WorkArrangementServiceImpl() {
    }
