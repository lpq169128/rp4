...
@Service
public class AntiAcciDrillServiceImpl extends WfAbstractServiceImpl<ModuleData> implements AntiAcciDrillService {
    public void fixWFBean(ModuleData bean, SecureUser operator) throws Exception {
        if (StringUtils.isBlank(bean.getBusinessUrl())) {
            bean.setBusinessUrl("pf/antiAcciDrill/moduleDataDetailPage.do?mode=view&moduleCode=" + bean.getBusinessType() + "&id=" + bean.getBusinessId());
        }
//        String desc = DataFormatUtil.substr(this.serviceUtil.fixExpressionFromBean(bean.getModule().getDescExpression(), bean, operator), Integer.valueOf(200));
        String desc = DataFormatUtil.substr("反事故演习记录", 200);
    ...
    public AntiAcciDrillServiceImpl() {
    }
