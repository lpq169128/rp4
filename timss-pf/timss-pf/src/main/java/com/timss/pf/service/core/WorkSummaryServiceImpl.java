...
@Service
public class WorkSummaryServiceImpl extends WfAbstractServiceImpl<ModuleData> implements WorkSummaryService {
    public void fixWFBean(ModuleData bean, SecureUser operator) throws Exception {
        if (StringUtils.isBlank(bean.getBusinessUrl())) {
            bean.setBusinessUrl("pf/workSummary/moduleDataDetailPage.do?mode=view&moduleCode=" + bean.getBusinessType() + "&id=" + bean.getBusinessId());
        }
//        String desc = DataFormatUtil.substr(this.serviceUtil.fixExpressionFromBean(bean.getModule().getDescExpression(), bean, operator), Integer.valueOf(200));
        String desc = DataFormatUtil.substr("工作总结", 200);
