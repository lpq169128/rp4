...
<head>
    ...
    <script>
        ...
        var initColumns = [[
            ...
            {
                field: 'mainForm_drillTime', title: '演习时间', width: 100, align: 'left', fixed: true,
            ...
            {field: 'mainForm_drillTitle', title: '演习题目', width: 160, align: 'left',
            formatter:function (value) {
                return value;
            }},
            {field: 'createUserName', title: '创建人', width: 100, align: 'left', fixed: true,},
            {
        ...
        $(document).ready(function () {
            PageList.init($.extend(true, PageModuleList.initParams(moduleList), {
                datagrid: {
                    ...
                    detailPage: {
                        url: basePath + "pf/antiAcciDrill/moduleDataDetailPage.do?mode=view&moduleCode=" + moduleList.businessType + "&id=",//required
                        createUrl: basePath + "pf/antiAcciDrill/moduleDataDetailPage.do?mode=create&moduleCode=" + moduleList.businessType + "&id="//required
                    },
                    params: {
        ...
        //设置新建权限
        function initPriv() {
            Priv.map("isEditable()", "antiAcciDrill_create");
            Priv.apply();
        }

        function isEditable() {
            var privMapping = _parent().privMapping;
            return !!privMapping["antiAcciDrill_list_create"];
        }
        //流程状态
...
<div class="toolbar-with-pager bbox">
    <div id="toolbar" class="btn-toolbar ">
        <div class="btn-group btn-group-sm" id="btnCreate">
            <button class="btn-success btn priv" privilege="antiAcciDrill_create" onclick="PageList.toCreate()">新建</button>
        </div>
