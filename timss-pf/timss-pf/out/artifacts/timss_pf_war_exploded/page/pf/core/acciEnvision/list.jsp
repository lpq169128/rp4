...
<head>
    ...
    <script>
        ...
        var initColumns = [[
            ...
            {
                field: 'mainForm_predictionTime', title: '预想时间', width: 100, align: 'left', fixed: true,
            ...
            {field: 'mainForm_predictionTitle', title: '预想题目', width: 160, align: 'left',
            formatter:function (value) {
                return value;
            }},
        ...
        $(document).ready(function () {
            PageList.init($.extend(true, PageModuleList.initParams(moduleList), {
                datagrid: {
                    ...
                    detailPage: {
                        url: basePath + "pf/acciEnvision/moduleDataDetailPage.do?mode=view&moduleCode=" + moduleList.businessType + "&id=",//required
                        createUrl: basePath + "pf/acciEnvision/moduleDataDetailPage.do?mode=create&moduleCode=" + moduleList.businessType + "&id="//required
                    },
                    params: {
        ...
        //设置新建权限
        function initPriv() {
            Priv.map("isEditable()", "acciEnvision_create");
            Priv.apply();
        }

        function isEditable() {
            var privMapping = _parent().privMapping;
            return !!privMapping["acciEnvision_list_create"];
        }
        //流程状态
        function getStatus(status, flowStatus) {
            ...
            if (status == "草稿") {
                value = "草稿";
            } else if (status == 'canyuzhequeding') {
                value = "参与者确定";
            } else if (status == 'zhuanyefuzerenshenpi') {
                value = "专业负责人审批";
            }else if (status == "已归档") {
                value = "已归档";
            } else if (status == "作废") {
...
<div class="toolbar-with-pager bbox">
    <div id="toolbar" class="btn-toolbar ">
        <div class="btn-group btn-group-sm" id="btnCreate">
            <button class="btn-success btn priv" privilege="acciEnvision_create" onclick="PageList.toCreate()">新建</button>
        </div>
