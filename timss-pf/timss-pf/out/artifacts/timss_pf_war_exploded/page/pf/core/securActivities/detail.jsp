...
<head>
    ...
    <script type="text/javascript">
        ...
        var planNameVal = userTeamMap != "" ? userTeamMap[userTeamID] + "组" : (userBranchMap != "" ? userBranchMap[userBranchID]
            : userResponsibilityMap[userResponsibilityID]);
        var wfStatus="";

        // var planName = year + "年" + month + "月" + planNameVal + "工作计划";

        // var month = {january:1, february:2, march:3, april:4, may:5, june:6,
        //     july:7, august:8, september:9, october:10, november:11, decembr:12}
        /**
         //
        ...
        $(document).ready(function () {
            ...
            PageDetail.init($.extend(true, PageModule.initParams(module,{
                isGridFieldCodeWithFormCode:false
            }), {
                page: page,
            ...
            $("#f_parPerson").parent().click(function () {
                getSucUser("mainForm","parPerson");
            })
            $("#f_absentee").parent().click(function () {
                getSucUser("mainForm","absentee");
            })
            //给暂存按钮增加点击事件，更改默认值。
        ...
        //参与人勾选
        function getSucUser(formId,fieldId) {
            setSucUserForDialog(formId,fieldId);
            console.log("getMgmtUser-----------");
            var src = basePath + "page/pf/core/antiAcciDrill/select_role_user.jsp";
            var btnOpts = [
                {
                    "name": "关闭",
                    "onclick": function () {
                ...
                },
                {
                    ...
                    "onclick": function () {
                        ...
                        saveSucUserForDialogToFrom(formId,fieldId);
        ...
        //为选人弹出框初始化准备数据
        function setSucUserForDialog(formId,fieldId) {
            var data = $('#' + formId).iForm('getVal');
            var participantIds = "";
            var participantName = "";
            if (fieldId=="parPerson"){
                ...
                if (typeof(data.absenteeUser) != "undefined" && data.absenteeUser != null && data.absenteeUser != "") {
                    participantIds = data.sucUser.split(",");
                    participantName = data.absentee.split(",");
                }
            }
            var schedulePerson = {};
        ...
        //保存修改的参与人
        function saveSucUserForDialogToFrom(formId,fieldId) {
            var strId = "";
            var strName = "";
            ...
            if(fieldId=="parPerson") {
                ...
                $("#" + formId).iForm("setVal", {"absentee": strName,"absenteeUser":strId});
            }
